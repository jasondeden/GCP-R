...
```{r}
#Convert pop_density to a numeric value (easier now that this is a factor instead of character)

print("Replace factor labels with numeric values")

as.numeric(localdata_minus_date$pop_density)

#Factors get converted to 1's and 2's. We might want this to be 0's and 1's. If so, and if we don't care
#which value gets coded as a 0, we can simply replace all the 2's with 0's. 

print("Replacing the 2's with 0's")

sapply(as.numeric(localdata_minus_date$pop_density), function(x) ifelse(x==2,0,1))

#However, if you wanted to make all the 2's 1's, and the 1's 0's, turns out the same basic
#function works with slight modification:

print("Replacing all 2's with 1's, and 1's, with 0's")
...
#tapply takes three main entries - the data you want to do something to, the value you want to sort by, and
...
```{r}
#Let's combine those results into a dataframe for easier comparison
...
surprise given our histogram (other than the shift to the left and the really long tail to the right, it did
...
tseries::jarque.bera.test(localdata$cases_per_1k)
...
The way we interpret that is as follows: For individuals living in sparsely populated counties, you have a 99% (or
a 1% lower) chance of dying of COVID-19 vs. individuals living in a densely populated county. This would align
with our base understanding of how diseases spread, perhaps - the closer together people are, the easier it is for
a virus to transmit from one person to another. So a drop in the rates of transmission and death are to be
expected.
...
(confirmed_cases and death values) are equal. Now that we've done all of this work, let's go back and check to
see if these base assumptions are accurate. If not, we can infer nothing from these results.

```{r}
print("Cases mean and variance")
mean(localdata$confirmed_cases, na.rm=T)
var(localdata$confirmed_cases, na.rm=T)

print("Deaths mean and variance")
mean(localdata$deaths, na.rm=T)
var(localdata$deaths, na.rm=T)

```

Oops. Those numbers are way too far apart for our Poisson distributed data assumption. Therefore, we have to discard our results and try something else. 

### Negative Binomial Model

We pivot to a Negative Binomial, from the MASS package, which is similar to the Poisson but does not make assumptions about mean and variance being equal in the predicted values. 
...
So it turns out our incorrect use of Poisson before ensuring its assumptions were met led us to EXACTLY the opposite of reality. If you live in a sparsely populated county, you are *MORE* likely to catch COVID-19 by about 5%, and more likely to die from COVID-19 by a whopping 14%!!


### Confidence Interval

The confidence interval.... how sure are we that our results are withing 2 standard deviations of the mean?
i.e. we will be within that range 95% of the time?
